...
boost::tuple<uint64_t,SubsecondTime> RobTimer::simulate(const std::vector<DynamicMicroOp*>& insts)
{
   ...
   for (std::vector<DynamicMicroOp*>::const_iterator it = insts.begin(); it != insts.end(); it++ )
   {
    ...
    if (deptrace_first_line) {
       DynamicMicroOp &dmo = *entry->uop;
      uint32_t instruction_address= dmo.getMicroOp()->getInstruction()->getAddress();
         ...
         if (deptrace_is_branch || deptrace_is_load || deptrace_is_store || (deptrace_reg_deps.size()!=0) || (deptrace_mem_deps.size()!=0) || (deptrace_addr_deps.size()!=0))
         {
            ...
            if (deptrace_is_load)
            {
               if (deptrace_last_was_newline)
                  ...
                  deptrace_f << " ";
               DynamicMicroOp &dmo = *entry->uop;
               uint32_t instruction_size= dmo.getMicroOp()->getInstruction()->getSize();
          ...
          deptrace_f << "t" << std::hex << (dmo.getBranchTarget()) <<std::dec;
      ...
      if ((*it)->getMicroOp()->is_trace)
      {
        ...
        deptrace_f << "\n";
         ...
         case 3:
            ...
            deptrace_save_data[0] = (*it)->getMicroOp()->trace_data[1];
            deptrace_save_data[1] = (*it)->getMicroOp()->trace_data[2];
            deptrace_save_data[2] = thread_id;
         ...
         case 4:
            ...
            assert(deptrace_acquire_list.count(deptrace_save_data[0]) == 0);
            // Insert this address
            deptrace_acquire_list.insert(deptrace_save_data[0]);

            break;
         // Initial Rel
         case 5:

            // We should have this address in here.
            assert(deptrace_acquire_list.count((*it)->getMicroOp()->trace_data[1]) == 1);
            // Remove this address
            deptrace_acquire_list.erase((*it)->getMicroOp()->trace_data[1]);
         ...
         // Final Cond Sig
         case 14:
            assert(deptrace_last_command == 13);
            deptraceSetActive(thread_id, true);
            break;
         // Cond BCast
         case 15:
         ...
         case 17:
            ...
            // We need to have the one entry in here
            assert(deptrace_acquire_list.count((*it)->getMicroOp()->trace_data[2])==1);
            // Release
            deptrace_acquire_list.erase((*it)->getMicroOp()->trace_data[2]);
            ...
            // Always handle thread state data
            assert(deptrace_thread_released.count(thread_id) == 0);
            deptrace_thread_released[thread_id] = (*it)->getMicroOp()->trace_data[2]; // mutex pointer
         ...
         case 18:
            ...
            // We shouldn't have this address in here.
            assert(deptrace_acquire_list.count(deptrace_thread_released[thread_id]) == 0);
            // Add this address
            deptrace_acquire_list.insert(deptrace_thread_released[thread_id]);

            deptraceSetActive(thread_id, true);

            // We need to explicitly check because we might not have been ROI/RMS yet
            if (deptraceIsActive(thread_id))
            {
            ...
            // Always handle thread state data
            assert(deptrace_thread_released.count(thread_id) == 1);
            deptrace_thread_released.erase(thread_id);
